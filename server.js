...
// Historical candles (for RVI/breadth analysis) — confirmed against Fyers'
// official Go SDK: StockHistoryURL = BaseDataURL + "/history?"
...
// Current F&O stock list — tries NSE's public market-lot CSV server-side
// (Node isn't subject to browser CORS, and this file needs no auth).
// NSE has changed/discontinued this exact file before, so this can fail —
// the frontend falls back to a bundled list if so.
  ...
  for (const url of candidates) {
    try {
      ...
      for (const line of lines.slice(1)) {
        ...
        const sym = cols[1];
        if (sym && sym !== 'SYMBOL' && !symbols.includes(sym)) symbols.push(sym);
      }
      if (symbols.length > 20) {
        return res.json({ s: 'ok', source: url, symbols });
      }
    } catch (e) { /* try next candidate */ }
  }
  res.json({ s: 'error', message: 'Could not fetch live F&O list from NSE — using bundled fallback list instead' });
});
